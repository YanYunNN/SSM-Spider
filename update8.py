...
class SaveLiterature:
    ...
    def get_lit_url(self, selector):
        ...
        while i < len(urls):
            ...
            print(use_number)
            download = url_selector.xpath('//div[@class="allversion_content"]//a[@class="dl_item"]/@href')
            download_name = url_selector.xpath('//div[@class="allversion_content"]//span[@class="dl_source"]/text()')
            print(len(download))
    ...
    def data_db(self, literature_name, author_, abstract_, publish_, publish_time, use_number):
        ...
        cursor = db.cursor()
        sql = "insert into lit_literature(literatureName,author,abstractText,publish,publishTime,useNumber)\
                   values('%s','%s','%s','%s','%s','%s')" % \
